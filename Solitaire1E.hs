...
{- COM2108 Solitaire assignment: 2018 assignment 2
   ...
   EXPERIMENTAL VERSION.. multiway if, fn composition, newType
-}
...
-- column aces to foundations    
 ...
 colAcesToFoundations (fnds,cols,res) = 
  ...
  (filter (not.null) (map (\col@(hc:tc) -> if 
                                            |isAce hc -> tc
                                            |otherwise -> col)
                           cols)),
   res)
...
-- column heads to foundations
 ...
 colHeadsToFoundations (fnds,cols,res) = 
  ((map (\f-> if 
               |((not.isKing) f ) &&(elem (sCard f) colHeads)->(sCard f)
               |otherwise->f)
         fnds), -- **f can't be king update foundations
   (filter (not.null) -- update columns
           (map (\col@(hc:rc)-> if
                                 |(elem (pCard hc) fnds)-> rc 
                                 |otherwise -> col)
                cols)), -- may have emptied a column
   res) 
  where
   colHeads = map head cols 
...
-- comprehension version
 ...
 colHeadsToFoundationsC (fnds,cols,res) = 
   (([(sCard f)|f<-fndsChanging]++ fndsUnchanged),
    ([ct|(_:ct)<-colsChanging, (not.null) ct]++colsUnchanged),
    res)
  where
   colHeads = map head cols
   fndsChanging = [f|f<-fnds, (not.isKing) f ,(elem (sCard f) colHeads)]
   fndsUnchanged = [f|f<-fnds, not (elem f fndsChanging)] 
   colsChanging = [c|c@(hc:_)<-cols, (elem (pCard hc) fnds)]
   colsUnchanged = [c|c<-cols, not (elem c colsChanging)]
